...
export function AgendaList({ appointments, onSelect }: Props) {
  ...
  if (appointments.length === 0) {
    return (
      <Text style={[styles.empty, { color: colors.muted }]}>Нет записей</Text>
    );
  }
  return (
    <ScrollView
          ...
          <Pressable key={a.id} onPress={() => onSelect(a)} style={styles.row}>
            ...
            <View style={[styles.rail, { backgroundColor: `${c}50` }]}>
              <View
                style={[
                  styles.railDot,
                  { backgroundColor: c, borderColor: colors.bg },
                ]}
              />
            </View>
            <View
            ...
            >
              <View style={styles.cardTop}>
                <Text numberOfLines={1} style={[styles.svc, { color: c }]}>
                  {a.serviceName}
                </Text>
              ...
              <View style={styles.cardBottom}>
                <Text style={[styles.client, { color: colors.textSoft }]}>
                  {a.clientLabel}
                </Text>
                <View
                  style={[
