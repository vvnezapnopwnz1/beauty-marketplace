...
import { publishMasterProfile } from "../../api/masterOnboarding";
import {
...
const DEFAULT_HOURS: WorkingHour[] = DAY_LABELS.map((_, i) => ({
  dayOfWeek: i,
...
export function StepScheduleScreen() {
  ...
  const validate = (): string | null => {
    for (const h of hours) {
      ...
      if (!isTimeValid(h.opensAt) || !isTimeValid(h.closesAt)) {
        return `Введите время в формате ЧЧ:ММ для дня: ${DAY_LABELS[h.dayOfWeek]}`;
      }
      if (timeToMinutes(h.opensAt) >= timeToMinutes(h.closesAt)) {
        return `Время начала должно быть раньше окончания для дня: ${DAY_LABELS[h.dayOfWeek]}`;
      }
    }
    return null;
  };
  ...
  const handleNext = async () => {
    ...
    setBusy(true);
    try {
      const update = await updateMutation.mutateAsync(hours);
      console.log("Updated schedule successfully:", update, hours);
  ...
  return (
    <OnboardingShell
      ...
      title="Расписание"
      subtitle="Настройте график работы"
    >
