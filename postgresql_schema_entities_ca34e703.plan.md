...
- **Мастера:** в MVP по [docs/context.md](docs/context.md) «несколько мастеров в салоне» нет в продукте, но сущность `staff` (мастер) даёт FK для `appointments` и не ломает UI «один салон = один поток записей» (одна запись по умолчанию на «дефолтного» мастера или `staff_id` nullable до появления UI).
