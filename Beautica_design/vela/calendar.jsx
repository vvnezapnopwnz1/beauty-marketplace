// VELA Calendar — improved week/month/day views with DnD reschedule
const { useState: uS, useRef: uR, useEffect: uE, useCallback: uC, createContext: uCC, useContext: uCX } = React;

// ── DnD Context ────────────────────────────────────────
const DnDCtx = uCC(null);

function DnDProvider({ children, onReschedule }) {
  const [dragging, setDragging] = uS(null); // appointment being dragged
  const [dropHover, setDropHover] = uS(false);
  const [showDialog, setShowDialog] = uS(false);

  const startDrag = uC((appt, e) => {
    e.dataTransfer.setData('text/plain', appt.id);
    e.dataTransfer.effectAllowed = 'move';
    // Small delay so the drag image renders before we set state
    setTimeout(() => setDragging(appt), 0);
  }, []);
  ...
  const handleDropOnZone = uC(() => {
    setDropHover(false);
    if (dragging) setShowDialog(true);
  }, [dragging]);

  const confirmReschedule = uC((newStart, newEnd) => {
    if (dragging && onReschedule) {
      onReschedule(dragging.id, newStart, newEnd);
    }
    setShowDialog(false);
    setDragging(null);
  }, [dragging, onReschedule]);

  const cancelDialog = uC(() => {
    setShowDialog(false);
    setDragging(null);
  }, []);

  return (
    <DnDCtx.Provider value={{ dragging, dropHover, showDialog, startDrag, endDrag, setDropHover, handleDropOnZone, confirmReschedule, cancelDialog }}>
      {children}
    </DnDCtx.Provider>
  );
}

// ── Reschedule Drop Zone (bottom strip) ────────────────
function RescheduleDropZone({ t }) {
  const { dragging, dropHover, setDropHover, handleDropOnZone } = uCX(DnDCtx);
  ...
  return (
    <div
      ...
      onDrop={e => { e.preventDefault(); handleDropOnZone(); }}
      style={{
        padding: '14px 20px',
        background: dropHover
          ? `linear-gradient(0deg, ${t.accent}30, ${t.accent}10)`
          : `linear-gradient(0deg, ${t.accentSoft}, transparent)`,
        border: `2px dashed ${dropHover ? t.accent : t.border}`,
        borderRadius: VELA.radius.md,
    ...
    >
      ...
      <span style={{
        ...
        color: dropHover ? t.accent : t.textSub,
        fontFamily: VELA.fonts.ui,
        transition: 'color 0.15s',
      }}>
        {dropHover ? `Отпустите, чтобы перенести «${dragging.service}»` : 'Перенести на другую дату…'}
      </span>
    </div>
  );
}
...
// ── Reschedule Dialog ──────────────────────────────────
function RescheduleDialog({ t }) {
  const { dragging, showDialog, confirmReschedule, cancelDialog } = uCX(DnDCtx);
  const [selDate, setSelDate] = uS(null);
  const [selTime, setSelTime] = uS(null);

  // Reset selections when dialog opens
  uE(() => {
    if (showDialog && dragging) {
      setSelDate(null);
      setSelTime(null);
    }
  }, [showDialog]);

  if (!showDialog || !dragging) return null;

  // Build a mini calendar for current + next month
  const today = new Date();
  const baseMonth = dragging.start.getMonth();
  const baseYear = dragging.start.getFullYear();

  // Generate 6 weeks of days starting from the 1st of current month
  ...
  const duration = dragging.end - dragging.start; // ms
  const origDateStr = dragging.start.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', weekday: 'short' });
  const selDateObj = selDate ? new Date(baseYear, selDate.month, selDate.day) : null;
  const selDateStr = selDateObj ? selDateObj.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', weekday: 'short' }) : null;
  ...
  const handleConfirm = () => {
    ...
    const newStart = new Date(baseYear, selDate.month, selDate.day, hh, mm);
    const newEnd = new Date(newStart.getTime() + duration);
    confirmReschedule(newStart, newEnd);
  };

  const color = serviceColor(t, dragging.cat);

  return (
    <div onClick={cancelDialog} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, backdropFilter: 'blur(6px)' }}>
      <div onClick={e => e.stopPropagation()} style={{
        background: t.surface, border: `1px solid ${t.border}`, borderRadius: VELA.radius.xl,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <span style={{ fontFamily: VELA.fonts.display, fontSize: 20, fontWeight: 500, color: t.text, letterSpacing: '-0.4px' }}>Перенести запись</span>
        </div>
        <div style={{ fontSize: 12, color: t.textMuted, marginBottom: 20, paddingLeft: 18 }}>
          {dragging.service} · {dragging.client} · {origDateStr} {fmtTime(dragging.start)}–{fmtTime(dragging.end)}
        </div>

        {/* Date picker */}
        <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>Выберите дату</div>
        <div style={{ background: t.surfaceEl, borderRadius: VELA.radius.md, padding: 12, marginBottom: 16, border: `1px solid ${t.borderSub}` }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: t.text, textAlign: 'center', marginBottom: 10 }}>
            {MONTH_NAMES[baseMonth]} {baseYear}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
            {DAYS_SHORT.map(d => (
              <div key={d} style={{ fontSize: 10, fontWeight: 600, color: t.textMuted, textAlign: 'center', padding: '4px 0' }}>{d}</div>
            ))}
            {calDays.map((cd, i) => {
              const isOrigDay = !cd.dim && cd.day === dragging.start.getDate() && cd.month === dragging.start.getMonth();
              const isSel = selDate && selDate.day === cd.day && selDate.month === cd.month;
              const isToday = !cd.dim && cd.day === today.getDate() && cd.month === today.getMonth() && baseYear === today.getFullYear();
              return (
                <div key={i} onClick={() => !cd.dim && setSelDate({ day: cd.day, month: cd.month })}
                  style={{
                    fontSize: 12, textAlign: 'center', padding: '6px 0', borderRadius: VELA.radius.sm,
                    cursor: cd.dim ? 'default' : 'pointer',
                    background: isSel ? t.accent : isOrigDay ? `${color}25` : 'transparent',
                    color: isSel ? '#fff' : cd.dim ? t.textMuted + '60' : isToday ? t.accent : t.text,
                    fontWeight: isSel || isToday ? 700 : 400,
                    border: isOrigDay && !isSel ? `1px dashed ${color}` : '1px solid transparent',
        ...
        {/* Time picker */}
        <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 8 }}>Выберите время</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 5, marginBottom: 20 }}>
          {timeSlots.map(ts => {
            const active = selTime === ts;
            return (
              <div key={ts} onClick={() => setSelTime(ts)}
                style={{
                  padding: '7px 4px', textAlign: 'center', fontSize: 12, borderRadius: VELA.radius.sm,
        ...
        {/* Summary */}
        {canConfirm && (
          <div style={{ padding: '10px 14px', background: t.accentSoft, borderRadius: VELA.radius.md, marginBottom: 16, fontSize: 12, color: t.accent, fontWeight: 500 }}>
            → {selDateStr}, {selTime} – {(() => {
              const [hh, mm] = selTime.split(':').map(Number);
              const ns = new Date(baseYear, selDate.month, selDate.day, hh, mm);
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <Btn variant="outlined" style={{ flex: 1 }} onClick={cancelDialog}>Отмена</Btn>
          <Btn variant="accent" style={{ flex: 2, opacity: canConfirm ? 1 : 0.4, pointerEvents: canConfirm ? 'auto' : 'none' }} onClick={handleConfirm}>Перенести</Btn>
        </div>
      </div>
    </div>
  );
...
function WeekView({ t, weekStart, onSelect }) {
  ...
  const [selectedMasters, setSelectedMasters] = uS(MASTERS.map(m => m.id));
  const dnd = uCX(DnDCtx);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: t.surface, borderRadius: VELA.radius.lg, border: `1px solid ${t.border}` }}>
            ...
            return (
              <div key={di} style={{ position: 'relative', borderLeft: `1px solid ${t.border}` }}>
                ...
                {dayAppts.map((a, ai) => {
                  ...
                  const isDragging = dnd.dragging && dnd.dragging.id === a.id;
                  return (
                    <div key={a.id}
                      draggable
                      onDragStart={e => dnd.startDrag(a, e)}
                      onDragEnd={() => dnd.endDrag()}
                      onClick={() => onSelect(a)}
                      style={{ position: 'absolute', top: top + 2, left: 3, right: 3, height: height - 4, borderRadius: VELA.radius.sm, background: isDragging ? `${color}08` : `${color}18`, borderLeft: `3px solid ${color}`, padding: short ? '3px 6px' : '5px 8px', cursor: 'grab', overflow: 'hidden', transition: 'all 0.15s', zIndex: isDragging ? 10 : 2, opacity: isDragging ? 0.4 : 1 }}>
      ...
      {/* DnD Reschedule drop zone */}
      <RescheduleDropZone t={t} />
    </div>
  );
}
                    ...
                    {mAppts.map(a => {
                      ...
                      return (
                        <div key={a.id}
                          ...
                          onDragEnd={() => dnd.endDrag()}
                          onClick={() => onSelect(a)}
                          style={{ position: 'absolute', top: top + 2, left: 4, right: 4, height: height - 4, borderRadius: VELA.radius.sm, background: isDragging ? `${color}08` : `${color}18`, borderLeft: `3px solid ${color}`, padding: short ? '4px 8px' : '6px 10px', cursor: 'grab', overflow: 'hidden', zIndex: isDragging ? 10 : 2, transition: 'background 0.15s', opacity: isDragging ? 0.4 : 1 }}>
...
function VelaCalendar({ t }) {
  ...
  const handleReschedule = uC((apptId, newStart, newEnd) => {
  ...
  return (
    <DnDProvider onReschedule={handleReschedule}>
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
      {/* Toolbar */}
      ...
      <RescheduleDialog t={t} />
    </div>
    </DnDProvider>
  );
}

Object.assign(window, { VelaCalendar, APPTS, MASTERS, serviceColor: window.serviceColor, DnDProvider, RescheduleDropZone, RescheduleDialog });
