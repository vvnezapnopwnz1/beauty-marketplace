...
- **Mobile — глобальное обновление до Expo SDK 55:** проведена подготовка к переходу на Expo SDK 55 (React 19.2.0, React Native 0.83.6). Разработан скрипт `mobile/scripts/switch-sdk.sh` и подготовлены парные файлы `package.json.54/55` и `package-lock.json.54/55` для быстрого и безопасного переключения сред.
- **Mobile — реструктуризация навигации и плоский роутинг:** проведена полная перестройка таб-бара (`mobile/app/(tabs)/_layout.tsx`). Вместо старых вкладок внедрены вкладки: `Сегодня` (index), `Календарь`, `Создать запись (appointment-new)`, `Клиенты` (clients), `Уведомления`, `Профиль`. Экранные формы создания сущностей (`new.tsx`) перенесены из глубоких путей внутрь таб-бара: `(tabs)/appointment-new.tsx`, `(tabs)/clients-new.tsx`, `(tabs)/services-new.tsx`.
- **Mobile — массовое и индивидуальное расписание:** переписан `ScheduleEditor.tsx` на мобилке. Введен переключатель режимов: «Одинаково везде» (массовое редактирование с пресетами времени `09:00-18:00` и т.д. для выбранных дней недели) и «По дням недели» (индивидуальное редактирование). Реализована валидация форматов времени (`ЧЧ:ММ`), проверка на логику (время начала раньше окончания) и полноценный вывод интерактивных ошибок.
- **Mobile — CRM личных клиентов и автокомплит:** реализован полноценный функционал управления частными клиентами мастера (`ClientDetailsTab`, `ClientEditTab`, `ClientSheet`). Внедрен переиспользуемый компонент `ClientAutocompleteField.tsx` для быстрого асинхронного поиска по CRM-базе мастера при создании или изменении записей.
- **Mobile — редактирование профиля и специализаций:** на мобилке добавлена вкладка настроек профиля и специализаций (`profile-info.tsx`), позволяющая выбирать категории через `ServiceCategoryPicker` и обновлять стаж работы мастера.
- **Backend — эндпоинты онбординга и сохранение вложенного профиля:** бэкенд расширен эндпоинтами шагов онбординга мастера (`/api/v1/master-dashboard/onboarding/start`, `/api/v1/master-dashboard/onboarding/step`, `/api/v1/master-dashboard/onboarding/publish`). Сервис `user_profile` и эндпоинт `PUT /api/v1/me` теперь полноценно обрабатывают вложенные поля мастер-профиля (`master.specializations` и `yearsExperience`), каскадно обновляя связи в GORM. Добавлен контроллер загрузки аватара мастера `/api/v1/master-dashboard/avatar`.

### Последние изменения (2026-05-11)

- **Профиль `/me` vs кабинет мастера:** вкладка «Общее» в `/me` оформлена карточкой в стиле «Профиля салона» (`FormSection`, `FormField`, палитра dashboard); специализации мастера — `ChipMultiSelect` по `GET /master-dashboard/service-categories` вместо строки через запятую. Редактирование специализаций и стажа перенесено в `/me`; в кабинете мастера (`MasterDashboardPage` → профиль) остаётся загрузка аватара + просмотр данных и кнопка в `/me`. `PUT /api/v1/me` нормализует `master.specializations` через `servicecategory.NormalizeParentSlugList` (как кабинет мастера), при неверных slug — `validation_failed` с полем `specializations`.
...
### Последние изменения (2026-05-07 follow-up)

- **Чат Phase 1 — follow-up (ветка `chore/chat-phase1-followup` поверх master):** спасены in-progress баг-фиксы (room ID/AccessToken через `uuid.New()`, фикс несуществующего `salon_members.status` фильтра + добавлена роль `admin`, переименование guest endpoint в `/chat/external/rooms/{token}`, видимость чата при редактировании записи); `useChatStream` переключён на fetch+ReadableStream через `authFetch` (auth-user) и plain fetch на новый guest-stream (анон). `AppointmentChatHook` подключён в `BookingService.CreateGuestBooking`, `DashboardService.UpdateAppointmentStatus` и `UpdateAppointment` (с новым event `appointment.rescheduled`, который раньше вообще не эмитился). Добавлен endpoint `GET /api/v1/chat/external/rooms/{token}/stream` с per-room subscriber registry в `ChatBroadcaster`. На фронте: i18n-ключи `chat.*`, `currentUserId` пробрасывается в `ChatBubble` для own/other разметки, в `/me` `AppointmentCard` теперь встроен collapsible-чат для активных записей. На мобайле: новый route `/chat/[appointmentId]`, кнопка «Чат» в `AppointmentQuickActionsSheet`, deep-link push'ей с `data.type === 'chat.message'` через `Notifications.addNotificationResponseReceivedListener`. Backend chat-payload обогащён полями `type` и `appointmentId`. Тесты, lint-полишинг, unread-counter и `react-native-sse` отложены в Phase 2C — см. [`product/chat-roadmap.md`](chat-roadmap.md).

### Последние изменения (2026-05-07)

- **Чат Phase 1 — внешний (первая итерация на ветке `feat/chat-phase1-external`):** добавлены таблицы `chat_rooms`/`chat_messages`/`chat_message_reads` (миграция `000034`), сервисный слой с RBAC от участников записи (`AppointmentChatResolver` через `salon_members` и `master_profiles`), правило «первого шага» для гостя, маскировка контактов (RU телефоны + мессенджеры) на storage-level, архиватор (cron в Fx) переводит чат в `readonly` через 24ч после `completed`. SSE-доставка через расширение `NotificationService.PublishEvent` с новым событием `chat.message` (без записи в `notifications`). Анонимный гость аутентифицируется по `access_token` комнаты. Frontend: FSD-entity `entities/chat` (RTK Query + `useChatStream`), фича `features/chat-window`, embed-аккордеон в `AppointmentDrawer` дашборда салона, standalone-страница `/chat/:accessToken`. Mobile: API-обёртки + polling-хук + `ChatScreen`/`ChatBubble`. Phase 2 (внутренний чат и «Задать вопрос») — отдельные планы, см. [`product/chat-roadmap.md`](chat-roadmap.md). Бэклог follow-up для Phase 1 (тесты, ревью, подключение `AppointmentChatHook`, /me-интеграция, i18n, SSE для guest, mobile screen-wiring) задокументирован там же.

### Последние изменения (2026-05-06)

- **React Native mobile app (Phase 3-5 completion):** вкладки `clients` и `services` переведены на API (`master-dashboard/clients`, `master-dashboard/services`), добавлены экраны `notifications` (polling fallback), `finances`, базовые admin-surfaces (`staff/schedule/salon`) через `X-Salon-Id` wrapper `mobile/src/api/dashboardClient.ts`, подключена регистрация Expo push token (`expo-notifications` → `POST /api/v1/devices`) после восстановления сессии; добавлены `useGuardedMutation`, smoke-скелет офлайн-кэша (`mobile/e2e/offline-cache.yaml`), `eas.json` и CI workflow `mobile-ci.yml`.
- **React Native mobile app (Phase 2 P2-1..P2-5):** добавлен backend endpoint `GET /api/v1/master-dashboard/appointments/heatmap?month=YYYY-MM` с ответом `{ month, days[], maxPerDay }` и той же базовой видимостью записей (`salon_masters.master_id` или личная запись по `master_profile_id`), что используется в списке. В мобильном календаре удалён legacy `CalendarGrid`, добавлены `MasterCalendar`, `MonthHeatmap`, `AppointmentQuickActionsSheet` (`@gorhom/bottom-sheet`) и optimistic-reschedule хук `useRescheduleAppointment` с rollback; `mobile/app/(tabs)/calendar.tsx` подключён к реальным данным `['appointments']` и `['appointmentsHeatmap',{month}]`.
- **React Native mobile app (P0 hardening):** добавлен app-lock через `expo-local-authentication` (`BiometricGate` в `mobile/app/_layout.tsx`) с повторной блокировкой после ~5 минут в фоне; заложен каркас i18n (`mobile/src/shared/i18n/i18n.ts`, `locales/ru.json`, инициализация в `AppProviders`); таб-навигация перестроена на 4 вкладки (`today`, `calendar`, `records`, `clients`) через Expo Router `Tabs`, добавлен общий `AppHeader`, экран `records.tsx`; добавлен guard-скрипт `mobile/scripts/no-inline-mocks.sh` и npm-команда `check:mocks`; удалён каталог `Beautica_design`.
...
- **React Native mobile app (master dashboard shell):** обновлена структура роутинга Expo Router в `mobile/app/(tabs)/` и `mobile/app/_layout.tsx` под рабочий каркас кабинета мастера: добавлены отдельные вкладки dashboard (`index`, `appointments`, `calendar`, `clients`, `services`), кастомные UI-компоненты карточек/календаря/статов и слой темы (`mobile/src/theme/*`, `mobile/src/components/ui/*`, `mobile/src/components/dashboard/*`). Это фиксирует переход от базового scaffold к навигационно-готовому shell-экрану.
...
- **Кабинет мастера — трекер финансов (frontend integration):** добавлена entity `frontend/src/entities/master-finances/` (RTK Query endpoints, slice фильтров, форматтер валют), новая страница `MasterFinancesPage` в секции `?section=finances` с карточками `доход/расход/прибыль`, графиками трендов и топ-услуг, CRUD расходов/категорий и экспортом отчёта. В `rtkApi` добавлены теги `FinanceSummary`, `FinanceCategories`, `FinanceExpenses`; стор расширен `masterFinances`.
...
- **Staff phone OTP verification:** добавлена верификация телефона мастера перед сохранением: миграция `000028_staff_phone_verification` (таблица `staff_phone_verifications`), GORM-модель `StaffPhoneVerification`, репозиторий `StaffPhoneVerificationRepository`, сервис `StaffPhoneOTPService` (request/verify/consume proof, dev bypass с кодом «1234»). Эндпоинты `POST /api/v1/dashboard/phone-otp/request` и `POST .../verify`; `CreateStaff` и `UpdateStaff` гейтят запись телефона через `ValidateAndConsumeProof` (UUID proof). Фронтенд: RTK Query мутации `requestStaffPhoneOtp`/`verifyStaffPhoneOtp` в `staffApi.ts`, inline OTP UI в `StaffFormModal` (SMS/Telegram toggle, код, success alert), кнопка «Сохранить» задизейблена пока номер не подтверждён; в edit-режиме существующий номер считается pre-verified.
