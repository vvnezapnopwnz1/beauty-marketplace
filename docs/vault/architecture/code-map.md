...
| Master self-onboarding (`/for-masters` → wizard) | `backend/internal/service/master_onboarding.go`, `backend/internal/controller/master_dashboard_controller.go` (`StartOnboarding` + cases `onboarding`, `publish`), `backend/migrations/000042_master_profiles_publishing.up.sql`, `backend/migrations/000043_master_profiles_onboarding_step.up.sql` | `frontend/src/pages/master-onboarding/`, `frontend/src/shared/api/masterOnboardingApi.ts`, `frontend/src/pages/for-masters/ui/HeroSection.tsx`, `frontend/src/pages/for-masters/ui/CtaFooterSection.tsx`, `frontend/src/shared/lib/safeRedirect.ts` |
## Связанные заметки
