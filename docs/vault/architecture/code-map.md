...
| Master self-onboarding (`/for-masters` → wizard) | `backend/internal/service/master_onboarding.go`, `backend/internal/controller/master_dashboard_controller.go` (`StartOnboarding` + cases `onboarding`, `publish`), `backend/migrations/000042_master_profiles_publishing.up.sql`, `backend/migrations/000043_master_profiles_onboarding_step.up.sql`, `backend/migrations/000044_service_category_i18n_titles.up.sql` (i18n колонок + нормализация `specializations`), `backend/internal/servicecategory/salontype.go` (legacy slug → `parent_slug`) | `frontend/src/pages/master-onboarding/`, `frontend/src/shared/api/masterOnboardingApi.ts`, `frontend/src/pages/for-masters/ui/HeroSection.tsx`, `frontend/src/pages/for-masters/ui/CtaFooterSection.tsx`, `frontend/src/shared/lib/safeRedirect.ts` |
| `/me` — блок мастер-профиля (`master.*`, published/onboarding) | `backend/internal/service/user_profile_service.go`, `repository/user_profile.go`, `persistence/user_profile_repository.go` | `frontend/src/shared/api/meApi.ts`, `frontend/src/pages/me/ui/sections/GeneralSection.tsx` |
| Mobile master onboarding + post-login stacks | — | `mobile/app/(onboarding)/`, `mobile/app/(global-tabs)/`, `mobile/src/features/onboarding/`, `mobile/src/features/landing/`, `mobile/src/api/masterOnboarding.ts`, `mobile/app/(auth)/login.tsx` |
## Связанные заметки
