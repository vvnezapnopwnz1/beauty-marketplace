...
> **Исторический план + заметки.** Текущее поведение поиска и API — в [`product/status.md`](../product/status.md), [`architecture/api-flows.md`](../architecture/api-flows.md) и коде (`search.go`, `SearchPage`). Этот файл полезен как контекст эволюции, не как единственная спека.
