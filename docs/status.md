...
> Дата: 2026-04-11 | Версия: pre-MVP (v0.1)
...
#### Review, WaitlistEntry *(схема есть, функционал не реализован)*
...
| **Dashboard** | ✅ API `GET/POST/PATCH/PUT/DELETE /api/v1/dashboard/...` (JWT + `salon_members` owner/admin); UI: обзор, календарь, записи, услуги, мастера, расписание салона, профиль. Нужна строка в `salon_members` — см. `docs/seed-dashboard-access.sql` |
...
**Бекенд:**
- БД: PostgreSQL, 6 миграций, schema production-ready
...
| Dashboard | Рабочий API + UI по фазе 1 | Drag-sort услуг, богатый календарь по слотам, полнотекст поиск клиента — позже |
...
- API для владельца: список записей, расписание, управление услугами
...
- Эндпоинты: список записей, статус (pending → confirm/cancel), расписание
- Frontend: Dashboard — базовый кабинет на `/api/v1/dashboard` (дальше: слоты, аналитика, inbox)
...
| Booking slot = "завтра 10:00" (заглушка) | 🟠 Средне | Реализовать реальный календарь |
