...
|------|-----------|-------------|
| `docs/context.md` | Продукт, рынок, MVP scope, монетизация | Понять бизнес-логику и приоритеты |
| `docs/status.md` | Что готово, что нет, техдолг | Понять текущее состояние, не сломать работающее |
| `docs/architecture.md` | Дерево файлов, роуты, модели, DI-граф | Главный справочник: знать где что лежит и как связано |
...
Следуй существующим паттернам проекта (изучи `docs/architecture.md` секция «Бэкенд»):
...
```
# Из docs/architecture.md → Persistence модели:
